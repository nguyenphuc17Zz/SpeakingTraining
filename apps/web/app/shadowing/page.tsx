...
export default function ShadowingPage() {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200 max-w-5xl mx-auto pb-8">
      {/* Hero / Import Section */}
      <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-4 sm:p-5 washi-texture shadow-2xs space-y-3">
        <div className="absolute -top-24 -right-24 h-56 w-56 rounded-full bg-enso-gradient opacity-20 pointer-events-none" />
        ...
        <div className="relative max-w-3xl space-y-1.5">
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-bold">
            ...
            <span>SHADOWING CINEMA STUDIO</span>
          </div>

          <h1 className="text-xl sm:text-2xl font-black tracking-tight text-foreground font-sans">
            Biến Mọi Video YouTube Thành <span className="text-primary">Phòng Thu Shadowing</span>
          </h1>
        ...
        <div className="space-y-2 max-w-3xl">
          <form onSubmit={handleImportSubmit} className="relative flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              <input
                ...
                className={cn(
                  "w-full h-10 px-3 rounded-xl border bg-background text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 transition-all",
                  existingVideo
            ...
            {existingVideo ? (
              <Button
                type="submit"
                variant="primary"
                className="h-10 px-5 rounded-xl font-bold text-xs shrink-0 bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs gap-1.5"
              >
              ...
              <Button
                ...
                className="h-10 px-5 rounded-xl font-bold text-xs shrink-0 shadow-xs gap-1.5"
              >
                <Plus className="h-3.5 w-3.5" />
                <span>Nhập Video AI</span>
              </Button>
            )}
          </form>
          ...
          {existingVideo && (
            <div className="flex items-center gap-2 p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-semibold animate-in fade-in">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <div>
            <h2 className="text-sm font-bold text-foreground font-sans">
              Thư Viện Video Của Bạn ({videos.length})
            </h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 px-4 rounded-2xl border border-dashed border-border/80 bg-card/40 space-y-2">
            <Youtube className="h-10 w-10 mx-auto text-muted-foreground/40" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {videos.map((vid) => (
              <div
                key={vid.id}
                className="group relative rounded-2xl bg-card/95 border border-border/80 hover:border-primary/40 hover:shadow-xs transition-all duration-200 overflow-hidden flex flex-col justify-between"
              >
