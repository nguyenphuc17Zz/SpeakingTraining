...
class LearnerProfileService:
    ...
    async def recalculate_profile(
        ...
        # 2a. Fluency Score (from reaction speeds)
        latencies = []
        for a in attempts:
        ...
        # Faster latency -> Higher fluency score (target < 1500ms = 90+)
        fluency_score = max(30.0, min(95.0, round(100.0 - (avg_latency / 3000.0 * 50.0), 1)))

        # 2b. Grammar Score (from success rates across attempts)
        ...
        # 2c. Pronunciation Score (from pronunciation attempts or pitch scores)
...
print("All Dynamic Profile files written successfully!")
'''

with open(r"E:\SpeakingTraining\apps\api\scripts\build_dynamic_profile.py", "w", encoding="utf-8") as f:
    f.write(PROFILE_SERVICE.strip() + "\n")
