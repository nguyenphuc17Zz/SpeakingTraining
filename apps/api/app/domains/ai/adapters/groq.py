...
class GroqAdapter(BaseHTTPAdapter, AIProvider):
    """Production adapter for Groq LPU API (OpenAI-compatible REST & SSE Streaming)."""

    provider_id: str = "groq"
    DEFAULT_MODEL: str = "llama-3.3-70b-versatile"
    BASE_URL: str = "https://api.groq.com/openai/v1/chat/completions"

    def __init__(self, timeout_seconds: float = 45.0, transport: httpx.AsyncBaseTransport | None = None):
        super().__init__(provider_id="groq", timeout_seconds=timeout_seconds)
    ...
    async def generate(self, request: AIRequest, api_key: str) -> AIResponse:
        if not api_key:
            raise ProviderAuthError("Groq API key is required", provider_id=self.provider_id)
        ...
        async with self._build_client() as client:
            try:
                ...
                raise ProviderTimeoutError(f"Groq request timed out after {self.timeout_seconds}s", provider_id=self.provider_id, raw_error=te)
            except httpx.RequestError as re:
                raise ProviderUnknownError(f"Network error connecting to Groq: {re}", provider_id=self.provider_id, raw_error=re)
        ...
        if response.status_code != 200:
            ...
            self._handle_http_error(response.status_code, err_data, default_msg=f"Groq API returned status {response.status_code}")
            ...
            async with self._build_client() as client:
                async with client.stream("POST", self.BASE_URL, headers=headers, json=payload) as response:
                    if response.status_code != 200:
                        ...
                        self._handle_http_error(response.status_code, err_data, default_msg=f"Groq streaming failed with status {response.status_code}")
                    ...
                    async for line in response.aiter_lines():
                        ...
                        if line.startswith("data: "):
                            ...
                            if "x_groq" in chunk_data and "usage" in chunk_data["x_groq"]:
                                final_usage = self._extract_usage(chunk_data["x_groq"])
                            elif chunk_data.get("usage"):
    ...
    async def list_models(self, api_key: str) -> list[ModelMetadata]:
        """Dynamically fetch active models supported by Groq API."""
        if not api_key:
        ...
        for m in raw_models:
            ...
            is_rec = "llama-3.3-70b" in lower_id or "llama-3.1-70b" in lower_id
