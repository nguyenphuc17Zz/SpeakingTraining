"""Keigo & Register Studio API — Mode 2 (thin wrapper over Learning Engine)."""
...
from app.domains.keigo.exercise_factory import TIMER_DEFAULTS, KeigoExerciseFactory
...
router = APIRouter(prefix="/keigo", tags=["Keigo Studio — Mode 2"])
...
async def generate_keigo_exercise_get(
...
async def generate_keigo_exercise(
    sub_mode: str = Query(default="keigo_transformation", description="keigo_*"),
    pressure_level: str = Query(default="normal"),
    difficulty: str | None = Query(default=None),
    timer_limit_ms: int | None = Query(default=None, ge=500, le=10000),
    ...
    valid_modes = {e.value for e in ExerciseType if e.value.startswith("keigo")}
    ...
    if not item_key:
        ...
        match = next((i for i in items if i.item_type == "politeness"), None)
        if match:
            item_key = match.key
        else:
            from app.domains.learning.models import LearningItem

            key = f"keigo.{sub_mode}.{eff_diff}"
            ...
            if not existing:
                new_item = LearningItem(
                    ...
                    title=f"Keigo — {sub_mode.replace('keigo_', '').title()} ({eff_diff})",
    ...
    exercise = Exercise(
        ...
        success_criteria=["Đúng register, đúng hướng kính ngữ, tự nhiên"],
        ...
        template_version="v1",
        generator_version="keigo.1.0.0",
        prompt_version=LearningPrompts.KEIGO_GEN_PROMPT_VERSION,
        provider="keigo_factory",
    ...
    if timer_limit_ms:
        exercise.extra_metadata["keigo_config"]["timer_limit_ms"] = timer_limit_ms
    db.add(exercise)
    await db.commit()
...
async def submit_keigo_attempt(
    exercise_id: str,
    ...
    result = await session_svc.submit_exercise_attempt(
        ...
        reflex_metrics=payload.get("keigo_metrics") or payload.get("reflex_metrics"),
        keigo_metrics=payload.get("keigo_metrics") or payload.get("reflex_metrics"),
...
async def get_keigo_progress(
    ...
    keigo_attempts = [a for a in attempts if (a.metrics_json or {}).get("keigo") is not None or (a.exercise and a.exercise.exercise_type.startswith("keigo"))]
    total = len(keigo_attempts)
    if total == 0:
        return {"user_id": user_id, "period": period, "total_attempts": 0, "accuracy_rate": 0.0, "avg_reaction_ms": None, "by_sub_mode": {}}
    successes = sum(1 for a in keigo_attempts if a.success)
